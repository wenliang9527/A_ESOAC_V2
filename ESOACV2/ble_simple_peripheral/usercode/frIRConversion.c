...
void ESOAAIR_IRsend(uint8_t keynumber)
{
    ...
    if ((slot.IR_learn_state & BIT(0)) == BIT(0)) {
        ...
        /* 发送后更新状态 */
        ESOAAIR_UpdateStatusByKey(keynumber);
    } else {
        co_printf("Please perform IR learn first\r\n");
    }
}
...
void ESOAAIR_UpdateStatusByKey(uint8_t keynumber)
{
    ...
    switch(keynumber) {
        case 0: /* 电源键 */
            ESAirdata.AIRStatus = (ESAirdata.AIRStatus == airSW_ON) ? airSW_OFF : airSW_ON;
            status_changed = true;
            break;
        ...
        case 2: /* 温度+ */
            if (ESAirdata.AIRTemperature < TEMP_MAX) {
        ...
        case 3: /* 温度- */
            if (ESAirdata.AIRTemperature > TEMP_MIN) {
                ESAirdata.AIRTemperature--;
                status_changed = true;
            }
            break;
        ...
        case 4: /* 风速 */
            ESAirdata.AIRWindspeed = (ESAirdata.AIRWindspeed + 1) % 3;
            status_changed = true;
            break;
    }
    
    if (status_changed) {
        co_printf("ESOAAIR_UpdateStatusByKey: Key %d, status: temp=%d, mode=%d, wind=%d, power=%s\r\n",
                  keynumber,
                  ESAirdata.AIRTemperature,
                  ESAirdata.AIRMODE,
                  ESAirdata.AIRWindspeed,
                  ESAirdata.AIRStatus == airSW_ON ? "ON" : "OFF");
